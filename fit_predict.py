...
def main():
    ...
    if args.test_mode:
        ...
        sub_train_indices = np.random.choice(range(X_size), size=int(X_size*0.01), replace=False)
        X_test_size = X_test.shape[0]
        sub_test_indices = np.random.choice(range(X_test_size), size=int(X_test_size*0.01), replace=False)
    else:
        X_size = X_train.shape[0]
